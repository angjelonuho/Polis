...
export default class GenerateBuildings extends React.Component {
  ...
  getPlaces = async (latitude, longitude) => {
    ...
    if (latitude !== undefined || longitude !== undefined) {
      ...
      const api_callCafe = await fetch(`https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${latitude},${longitude}&radius=1500&type=cafe&key=${API_KEY}&libraries=places`);
      const dataCafe = await api_callCafe.json();

      const api_callRestaurant = await fetch(`https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${latitude},${longitude}&radius=1500&type=restaurant&key=${API_KEY}&libraries=places`);
      const dataRestaurant = await api_callRestaurant.json();

      const api_callBar = await fetch(`https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=${latitude},${longitude}&radius=1500&type=bar&key=${API_KEY}&libraries=places`);
  ...
  getPlacesDetails = async (arrayPlaceIds) => {
    ...
    for (let i = 0; i <= 17; i++) {
      ...
      api_Details = await fetch(`https://maps.googleapis.com/maps/api/place/details/json?place_id=${arrayPlaceIds[i]}&key=${API_KEY}&libraries=places`);
                        ...
                        <Row className="show-grid">
                          ...
                          <Col xs={12} md={12} sm={12} lg={6} xl={6}>
                            <Tab.Container id="left-tabs-example" defaultActiveKey="first">
                              <p className="text-center reviewsComments">Reviews</p>
                              <Tab.Content>
