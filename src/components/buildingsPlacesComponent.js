...
export default class GenerateBuildings extends React.Component {
  ...
  getPlaces = async (latitude, longitude) => {
    ...
    if (latitude !== undefined || longitude !== undefined) {
      ...
      const api_callCafe = await fetch(`/maps/api/place/nearbysearch/json?location=${latitude},${longitude}&radius=1500&type=cafe&key=${API_KEY}`, {mode: 'no-cors'});
      const dataCafe = await api_callCafe.json();

      const api_callRestaurant = await fetch(`/maps/api/place/nearbysearch/json?location=${latitude},${longitude}&radius=1500&type=restaurant&key=${API_KEY}`, {mode: 'no-cors'});
      const dataRestaurant = await api_callRestaurant.json();

      const api_callBar = await fetch(`/maps/api/place/nearbysearch/json?location=${latitude},${longitude}&radius=1500&type=bar&key=${API_KEY}`, {mode: 'no-cors'});
  ...
  getPlacesDetails = async (arrayPlaceIds) => {
    ...
    for (let i = 0; i <= 17; i++) {
      ...
      api_Details = await fetch(`/maps/api/place/details/json?place_id=${arrayPlaceIds[i]}&key=${API_KEY}`, {mode: 'no-cors'});
