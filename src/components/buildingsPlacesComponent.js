...
export default class GenerateBuildings extends React.Component {
  ...
  getPlaces = async (latitude, longitude) => {
    ...
    if (latitude !== undefined || longitude !== undefined) {
      ...
      const dataCafe = await api_callCafe.jsonp();
      ...
      const dataRestaurant = await api_callRestaurant.jsonp();
      ...
      const databar = await api_callBar.jsonp();
  ...
  getPlacesDetails = async (arrayPlaceIds) => {
    ...
    for (let i = 0; i <= 17; i++) {
      ...
      DeData = await api_Details.jsonp();
